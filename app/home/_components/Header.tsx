...
export default function Header() {
  ...
  return (
    <header className="fixed top-5 left-1/2 z-50 w-full -translate-x-1/2 px-4">
      <div className="mx-auto flex h-[76px] w-full max-w-[1380px] items-center justify-between rounded-full border border-white/20 bg-white/70 px-4 shadow-lg backdrop-blur-2xl md:px-7">
        {/* LEFT - Logo */}
        <Link
          href="/"
        ...
        <div className="flex items-center gap-3">
          ...
          {!isSubscription && (
            <Link
              href="/diagnostic"
              className="group flex items-center gap-2 rounded-full bg-gradient-to-r from-[hsl(187,27%,40%)] to-[hsl(45,93%,47%)] px-6 py-3 text-[12px] font-medium uppercase tracking-[0.18em] text-white shadow-lg transition-all duration-300 hover:scale-[1.02] hover:shadow-xl active:scale-[0.98]"
            >
          ...
          {/* Mobile Menu */}
          {isHome && (
            <button className="flex h-11 w-11 items-center justify-center rounded-full border border-[hsl(187,27%,40%)]/20 bg-white text-[hsl(187,27%,40%)] transition-all duration-300 hover:bg-[hsl(187,27%,40%)]/5 lg:hidden">
